...
module.exports = function(RED) {
    ...
    function CoEInputNode(config) {
        ...
        node.receiveAll = config.receiveAll || false;
        }
        ...
        const listener = (data) => {
            // Filter: IP-Adresse
            if (!node.receiveAll && node.cmiAddress && data.sourceIP !== node.cmiAddress) {
                return;
            }
            
            // Filter: Knoten-Nummer
            if (!node.receiveAll && node.nodeNumber > 0 && data.nodeNumber !== node.nodeNumber) {
                return;
            }
            ...
            const msg = {
                payload: value,
                topic: `coe/${node.nodeNumber}/${node.dataType}/${node.outputNumber}`,
                coe: {
            ...
            node.status({
                fill:"green", 
                shape:"dot", 
                text:`${value} ${unitInfo.symbol || ''} [${data.version}]`
            });
        };
    ...
    function parseCoEPacket(buffer, version) {
        ...
        // Block 1-8: Analog (4 Werte)
            ...
            if (version === 2 && buffer.length === COE_V2_ANALOG_PACKET_SIZE) {
                ...
                for (let i = 0; i < 4; i++) {
                    const value = buffer.readInt32BE(2 + i * 4); // 4 Bytes pro Wert
                ...
                for (let i = 0; i < 4; i++) {
                    const value = buffer.readInt16BE(2 + i * 2); // 2 Bytes pro Wert
